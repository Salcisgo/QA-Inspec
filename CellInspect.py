...
def on_serial_entered(event):
    global serial, user,password, operator_message
    ...
    if not re.fullmatch(r"K[A-Z0-9]{15}", serial):
        ...
        """
        operator_message.set = tk.StringVar(value="El número de serie debe empezar con 'K' y tener 16 caracteres alfanuméricos.\nEjemplo: K1045D22P1002781")
    ...
    if result["status"] != "ok":
        ...
        serial_entry.focus()
        return
    operator_message.set = tk.StringVar(value="Listo para iniciar inspección.")
...
def on_pass_clicked():
    ...
    if "success" in sendpass_response.lower():
        ...
        operator_message.set = tk.StringVar(value="Inspección registrada como PASA")
...
def on_fail_clicked():
    ...
    if "success" in sendfail_response.lower():
        ...
        operator_message.set= tk.StringVar(value="Inspección registrada como FALLA")
...
root = tk.Tk()
root.title("QA INSPEC 1.1")
root.geometry("500x300")  # Ajustar el tamaño para acomodar los nuevos elementos
...
# User label
user_top_label = tk.Label(root, text=f"Usuario: {user}", font=("Arial", 12, "bold"), fg="#080808")
...
# =======================
#  Caja de mensajes para el operador
# =======================

message_frame = tk.Frame(root)
message_frame.pack(padx=25, pady=10)
...
#efect_entry = ttk.Combobox(root, font=("Arial", 14), values=defectos, state="disabled")
...
operator_message = tk.StringVar(value="Seleccione Shop y Unique Id.")
message_box = tk.Label(
    message_frame,
    textvariable=operator_message,
    font=("Arial", 18, "bold"),
    fg="#1a237e",
    bg="#e3eafc",
    width=40,
